...
namespace FirefliesTwoO
{
    public class MeshOverlayDrawer
    {
        ...
        public static void DrawMeshArea(List<IntVec3> cells)
        {
            if (cells.Count > 0)
            {
                DrawMeshBounds(cells, ColorManager.BlueEmission);
            }
        }

        private static void DrawMeshBounds(List<IntVec3> cells, Color color, float? altOffset = null)
        {
            Map currentMap = Find.CurrentMap;
            Material material = CreateEdgeMaterial(color);
            InitializeFieldGrid(currentMap);
            float altitudeOffset = altOffset ?? GetRandomOffsetForColor(color);
        ...
        {
            return Rand.ValueSeeded(color.ToOpaque().GetHashCode()) * (1f / 26f) / 10f;
        }

        private static void MarkCellsInGrid(List<IntVec3> cells, Map map)
        {
            foreach (IntVec3 cell in cells.Where(cell => cell.InBounds(map)))
            {
                fieldGrid[cell.x, cell.z] = true;
            }
        }

        private static void DrawEdgesForCells(List<IntVec3> cells, Map map, Material material, float altitudeOffset)
        {
            ...
            foreach (IntVec3 cell in cells)
            {
                ...
                for (int direction = 0; direction < 4; direction++)
                {
                    ...
                    Vector3 drawPos = cell.ToVector3ShiftedWithAltitude(AltitudeLayer.MetaOverlays) + new Vector3(0f, altitudeOffset, 0f);
                    Graphics.DrawMesh(MeshPool.plane10, drawPos, new Rot4(direction).AsQuat, material, 0);
                }
            }
        }
    }
